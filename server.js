  ...
  exec(curlCommand, (error, stdout, stderr) => {
    ...
    fs.readFile('response.txt', 'utf8', (err, data) => {
      if (err) {
        ...
        return res.status(500).json({ error: 'Error reading response.txt' });
      }
      
      // Send the content of response.txt in the HTTP response
      return res.send({ message: 'Curl command executed successfully', output: data });
    });
  });
});
